...
def imprimir_heroe(heroe: list):
    """Muestra la información de un héroe de forma amena."""
    imprimir_separador()
...
def ordenar_por_nombre_burbuja(lista: list) -> list:
    ...
    usando el algoritmo de burbuja (sin librerías externas).
    Retorna una nueva lista ordenada para no modificar el original in-place
    sin confirmación (aunque en este contexto sí la modifica al ser la misma referencia).
    """
    n = len(lista)
    for i in range(n - 1):
