...
def imprimir_heroe(heroe: list):
    ...
    Identidad    : {heroe[INDICE_IDENTIDAD]}
    Empresa      : {heroe[INDICE_EMPRESA]}
    Altura       : {heroe[INDICE_ALTURA]:.2f} cm
    Peso         : {heroe[INDICE_PESO]:.2f} kg
    Género       : {heroe[INDICE_GENERO]}
    Color ojos   : {heroe[INDICE_COLOR_OJOS]}
    Color pelo   : {heroe[INDICE_COLOR_PELO]}
    Fuerza       : {heroe[INDICE_FUERZA]}
    Inteligencia : {heroe[INDICE_INTELIGENCIA]}
    """)
    imprimir_separador()
...
def imprimir_menu():
    ...
    2. Agregar héroe
    3. Eliminar héroe por nombre
    4. Ordenar héroes por nombre (A → Z)
    5. Ver héroe más alto
    6. Ver héroe más fuerte
    7. Ver héroe menos pesado
    0. Salir")
    """)
    imprimir_separador()
...
def buscar_maximo_recursivo(lista: list, indice_campo: int, indice_actual: int, indice_maximo: int) -> int:
    ...
    if indice_actual == len(lista): #devuelve el indice maximo
        return indice_maximo
    if lista[indice_actual][indice_campo] > lista[indice_maximo][indice_campo]: #mayor que 
...
def buscar_minimo_recursivo(lista: list, indice_campo: int, indice_actual: int, indice_minimo: int) -> int:
    ...
    if lista[indice_actual][indice_campo] < lista[indice_minimo][indice_campo]: #menor que
...
def buscar_heroe_por_nombre_recursivo(lista: list, nombre: str, indice: int) -> int:    
    ...
    if indice == len(lista):
        return -1 #funciona para indicar que no se encontró el héroe después de revisar toda la lista
...
def agregar_heroe(lista: list):
    ...
    lista.append(nuevo_heroe) #pasa de 25 a 26
